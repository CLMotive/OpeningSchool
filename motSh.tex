...
Then
\[
M(G) \isoto \left( \bigoplus_{i=0}^{?}\Sym_i(M_1(G)) \right) \otimes M(\pi_0(G/R)]). % TODO
\]
\end{theorem}
...
\end{enumerate}
\end{lemma}
Let $f:Y\to X$ be a separated morphism of finite type, then there exists a Nagata
compactification where $f$ factors as
\[
...
commutes with all coproducts. So by the Adjoint Functor Theorem, $f_!$ has a right
...
\end{enumerate}
\end{theorem}
\begin{exercise}[Zariski separation]
...
stable under composition and pullbacks. $\cat{Span}(\C,E)$ is the $\infty$-category of \defn{spans}:
\begin{itemize}
...
A \defn{3-functor formalism} is a $\infty$-symmetric monoidal functor
\[
...
\end{enumerate}
Taking $D_{=0} = D_{\ge 0}\cap D_{\le 0}$ gives us the \defn{heart} which is an abelian category.
\end{definition}
\begin{example}
Let
\[
