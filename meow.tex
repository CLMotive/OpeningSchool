...
\begin{theorem}[Straightening-Unstraightening Theorem]
	...
	\] 
	between the $\infty$-category of cocartesian fibrations over $\cat{D}$ and the $\infty$-category of functors $\cat{D}\to \cat{CaT}_\infty$. The forward map is called \defn{straightening} and the inverse is called \defn{unstraightening}.
\end{theorem}
...
Let's introduce the \defn{twisted arrow category} $\cat{Tw}(\C)$ for an $\infty$-category $\C$. We define
\[
...
\begin{definition}
	Let $\C$ be an $\infty$-category. A diagram of shape $\Delta^{1}\times \Delta^{1}$, say (Picture) is called a \defn{pullback} if every extension problem (Diagram) admits a solution.
\end{definition}
...
1-categorical limits are unique up to unique isomorphism, but $\infty$-categorical limits it's unique up to a contractible Kan complex of ($\infty$-categorical) isomorphisms.
...
\begin{lemma}
	...
	respectively.
\end{lemma}
The first functor is denoted $\Omega$ or $[-1]$ and called the \defn{loop space} functor. The second functor is denoted $\Sigma$ or $[1]$ and called the \defn{suspension} functor.
\begin{remark}
